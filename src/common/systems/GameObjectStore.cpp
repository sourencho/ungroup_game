...
GameObjectStore::GameObjectStore(PhysicsController& pc) : m_physicsController(pc) {
    m_players.reserve(GAME_SETTINGS.MAX_PLAYER_COUNT);
    m_groups.reserve(GAME_SETTINGS.MAX_PLAYER_COUNT);
    m_mines.reserve(GAME_SETTINGS.MAX_MINE_COUNT);

    // Initialize Players
    for (int i = 0; i < GAME_SETTINGS.MAX_PLAYER_COUNT; i++) {
